...
export class TransformInterceptor<T> implements NestInterceptor {
    ...
    intercept(context: ExecutionContext, next: CallHandler<any>): Observable<T> | Promise<Observable<T>> {
            ...
            map((data) => { // RxJS map transforms the entire emitted response (whether it's single object or array of objects)
                ...
                if (data && typeof data === 'object' && ('data' in data || 'message' in data)) { //data is the returend object{} than containes { data: , message:}
                    if (data.data) {
                        return {
                            ...data,// message
                            data: plainToInstance(this.classType, toPlain(data.data)) // returend data in the object
                        };
                    }
                    return data;
                }
                ...
                if (Array.isArray(data)) {
                    return plainToInstance(this.classType, data.map(toPlain));
                }
                ...
                if (data.user) {
                    return {
                        ...data,
                        user: plainToInstance(this.classType, toPlain(data.user))
                    };
                }

                // Otherwise, transform the single response object into a User instance
                return plainToInstance(this.classType, toPlain(data));
            })
        );
    }
}
